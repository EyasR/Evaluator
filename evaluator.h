...
class Evaluator {
 public:
  Evaluator( const string &s ) : str( s ) { // constructor receives an exp
    ...
    PREC_TABLE[26].inputSymbol = 1;    // ASGN
 ...
 private:
  ...
  // new member functions from program 6 specs
  vector<char> postFixVarStack;             // Postfix Machine stack for var
  NumericType var_a = 0;                    // variable a
  NumericType var_b = 0;                    // variable b
  NumericType var_c = 0;                    // variable c
  ...
  char getTopVar( );                        // Get top of postfixVar stack
};
